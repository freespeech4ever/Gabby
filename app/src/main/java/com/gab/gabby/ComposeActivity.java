...
import com.gab.gabby.adapter.ComposeAutoCompleteAdapter;
import com.gab.gabby.adapter.EmojiAdapter;
import com.gab.gabby.adapter.OnEmojiSelectedListener;
import com.gab.gabby.db.AccountEntity;
import com.gab.gabby.db.AppDatabase;
import com.gab.gabby.db.InstanceEntity;
...
import com.gab.gabby.network.ProgressRequestBody;
import com.gab.gabby.service.SendPostService;
import com.gab.gabby.util.ComposeTokenizer;
import com.gab.gabby.util.CountUpDownLatch;
import com.gab.gabby.util.DownsizeImageTask;
import com.gab.gabby.util.ListUtils;
import com.gab.gabby.util.SavePostHelper;
import com.gab.gabby.util.SpanUtilsKt;
import com.gab.gabby.util.StringUtils;
import com.gab.gabby.util.ThemeUtils;
import com.gab.gabby.view.ComposeOptionsListener;
import com.gab.gabby.view.ComposeOptionsView;
import com.gab.gabby.view.EditTextTyped;
import com.gab.gabby.view.ProgressImageView;
import com.gab.gabby.view.PostButton;
...
import static com.gab.gabby.util.MediaUtilsKt.MEDIA_SIZE_UNKNOWN;
import static com.gab.gabby.util.MediaUtilsKt.getImageSquarePixels;
import static com.gab.gabby.util.MediaUtilsKt.getImageThumbnail;
import static com.gab.gabby.util.MediaUtilsKt.getMediaSize;
import static com.gab.gabby.util.MediaUtilsKt.getSampledBitmap;
import static com.gab.gabby.util.MediaUtilsKt.getVideoThumbnail;
...
public final class ComposeActivity
    ...
    private static final String POST_TEXT_EXTRA = "post_text";
    ...
    private static final String POST_VISIBILITY_EXTRA = "post_visibility";
    ...
    // https://github.com/postsuite/mastodon/blob/1656663/app/models/media_attachment.rb#L94
    ...
    private int maximumPostCharacters = STATUS_CHARACTER_LIMIT;
    private @Px
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (theme.equals("black")) {
            setTheme(R.style.GabbyDialogActivityBlackTheme);
        }
        ...
        if (activeAccount != null) {
            ...
            mastodonApi.getInstance().enqueue(new Callback<Instance>() {
                ...
                public void onResponse(@NonNull Call<Instance> call, @NonNull Response<Instance> response) {
                    if (response.isSuccessful() && response.body().getMaxPostChars() != null) {
                        maximumPostCharacters = response.body().getMaxPostChars();
        ...
        postButton.setOnClickListener(v -> onSendClicked());
        ...
        if (intent != null) {
            ...
            String postText = intent.getStringExtra(POST_TEXT_EXTRA);
            if (!TextUtils.isEmpty(postText)) {
                textEditor.setText(postText);
            }

            // try to redo a list of media
            // If come from SavedPostActivity
            ...
            if (savedPostUid != 0) {
                ...
                // If come from SavedPostActivity
                startingText = postText;
            }

            int postVisibility = intent.getIntExtra(POST_VISIBILITY_EXTRA, Status.Visibility.UNKNOWN.getNum());
            if (postVisibility != Status.Visibility.UNKNOWN.getNum()) {
                startingVisibility = Status.Visibility.byNum(postVisibility);
            }
        ...
        if (mediaQueued.size() == 0) {
            ...
            if (statusMarkSensitive) {
                ...
                if (statusHideText) {
                    ...
                    color = ContextCompat.getColor(this, R.color.gabby_blue);
                }
            } else {
                hideMediaToggle.setClickable(true);
    ...
    private void setStatusVisibility(Status.Visibility visibility) {
        ...
        postButton.setStatusVisibility(visibility);
    ...
    private void updateVisibleCharactersLeft() {
        this.charactersLeft.setText(String.format(Locale.getDefault(), "%d", maximumPostCharacters - calculateTextLength()));
    }
    ...
    private void sendStatus(String content, Status.Visibility visibility, boolean sensitive,
        ...
        Intent sendIntent = SendPostService.sendPostIntent(this, content, spoilerText,
                ...
                accountManager.getActiveAccount(), savedPostUid);
    ...
    private void onReadySuccess(Status.Visibility visibility, boolean sensitive) {
        ...
        if ((characterCount <= 0 || contentText.trim().length() <= 0) && mediaQueued.size() == 0) {
            ...
            enableButtons();
        } else if (characterCount <= maximumPostCharacters) {
    ...
    private File createNewImageFile() throws IOException {
        ...
        String imageFileName = "Gabby_" + randomId + "_";
    ...
    private void saveDraftAndFinish() {
        ...
        savePostHelper.savePost(textEditor.getText().toString(),
    ...
    private void loadCachedInstanceMetadata(@NotNull AccountEntity activeAccount) {
        ...
        if (instanceEntity != null) {
            Integer max = instanceEntity.getMaximumPostCharacters();
            maximumPostCharacters = (max == null ? STATUS_CHARACTER_LIMIT : max);
    ...
    private void cacheInstanceMetadata(@NotNull AccountEntity activeAccount) {
        InstanceEntity instanceEntity = new InstanceEntity(activeAccount.getDomain(), emojiList, maximumPostCharacters);
        database.instanceDao().insertOrReplace(instanceEntity);
    }

    // Accessors for testing, hence package scope
    int getMaximumPostCharacters() {
        return maximumPostCharacters;
    }
    ...
    public static final class IntentBuilder {
        ...
        public IntentBuilder postText(String postText) {
        ...
        public Intent build(Context context) {
            ...
            if (postText != null) {
                intent.putExtra(POST_TEXT_EXTRA, postText);
            }
            ...
            if (visibility != null) {
                intent.putExtra(POST_VISIBILITY_EXTRA, visibility.getNum());
            }
            if (contentWarning != null) {
